...
export const ChooseYourMood: React.FC<ChooseYourMoodProps> = ({ onOpenBooking }) => {
  ...
  return (
    <section id="mood" className="relative min-h-[900px] flex items-center py-28 md:py-36 bg-[#0B0B0B] text-[#FAF8F3] overflow-hidden">
      ...
      {MOOD_OPTIONS.map((mood) => (
        <div
          ...
          className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${
            activeMoodId === mood.id ? 'opacity-35 scale-100' : 'opacity-0 scale-105 pointer-events-none'
          }`}
        >
          <img
            src={mood.bgImage}
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10 w-full">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <p className="font-sans text-sm sm:text-base text-[#F4F0E8]/70 font-light mt-3">
            Select your desired psychological and somatic state to unveil your bespoke therapeutic journey.
          </p>
        </div>
          ...
          {MOOD_OPTIONS.map((mood) => {
            ...
            return (
              <button
                ...
                data-cursor="pointer"
                className={`group py-5 px-4 sm:px-6 text-center border transition-all duration-500 cursor-pointer relative overflow-hidden ${
                  isActive
              ...
              >
                <span className={`font-serif text-lg sm:text-xl md:text-2xl tracking-widest uppercase block font-medium transition-colors ${
                  isActive ? 'text-[#0B0B0B]' : 'text-[#FAF8F3] group-hover:text-[#C8A96B]'
                }`}>
                  {mood.label}
                </span>
                <span className={`font-sans text-[10px] tracking-wider uppercase block mt-1 transition-colors ${
        ...
        <div className="max-w-4xl mx-auto bg-[#151515]/90 backdrop-blur-xl border border-[#C8A96B]/40 p-8 sm:p-12 shadow-2xl transition-all duration-700">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            ...
            <div className="md:col-span-7 space-y-4">
              ...
              <h3 className="font-serif text-3xl sm:text-4xl text-[#FAF8F3] uppercase leading-snug">
                {currentMood.title}
              </h3>

              <p className="font-sans text-sm text-[#F4F0E8]/80 font-light leading-relaxed">
                {currentMood.subtext}
              </p>

              <div className="pt-4 space-y-2 text-xs font-sans">
                <div className="flex items-center space-x-3 text-[#FAF8F3]/90">
                  <span className="text-[#C8A96B] uppercase tracking-wider font-semibold min-w-[110px]">Aroma Notes:</span>
                  <span className="font-light">{currentMood.aromaNote}</span>
                </div>
                <div className="flex items-center space-x-3 text-[#FAF8F3]/90">
                  <span className="text-[#C8A96B] uppercase tracking-wider font-semibold min-w-[110px]">Modality:</span>
            ...
            <div className="md:col-span-5 bg-[#0B0B0B] p-6 sm:p-8 border border-[#C8A96B]/30 flex flex-col justify-between space-y-6">
              <div>
                <span className="font-sans text-[10px] uppercase tracking-[0.25em] text-[#C8A96B] block mb-1">
                  PRESCRIBED TREATMENT
                </span>
                <h4 className="font-serif text-xl sm:text-2xl text-[#FAF8F3] uppercase">
                  {currentMood.recommendedTreatment}
                </h4>
                <div className="flex items-baseline space-x-3 mt-2 text-[#C8A96B]">
                  <span className="font-serif text-2xl font-bold">{currentMood.treatmentPrice}</span>
              ...
              <button
                ...
                data-cursor="book"
                className="w-full py-3.5 bg-[#C8A96B] hover:bg-[#D8C08A] text-[#0B0B0B] font-sans text-xs uppercase tracking-[0.2em] font-bold flex items-center justify-center space-x-2 transition-all duration-300 shadow-lg cursor-pointer"
              >
