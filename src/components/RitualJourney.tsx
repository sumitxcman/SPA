...
export const RitualJourney: React.FC = () => {
  return (
    <section id="rituals" className="relative py-28 md:py-36 bg-[#0B0B0B] text-[#FAF8F3] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-20">
          ...
          <p className="font-sans text-sm sm:text-base text-[#F4F0E8]/70 font-light mt-3">
            An uninterrupted sequence engineered to gently detach consciousness from modern velocity.
          </p>
        </div>
          ...
          {RITUAL_JOURNEY_STEPS.map((stepItem, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#0B0B0B] border-2 border-[#C8A96B] items-center justify-center z-20 shadow-[0_0_15px_rgba(200,169,107,0.3)]">
                ...
                <div className="w-full md:w-1/2" data-cursor="view">
                  <div className="relative group overflow-hidden border border-[#C8A96B]/30 shadow-2xl">
                    <img
                      src={stepItem.image}
                      alt={stepItem.title}
                      className="w-full h-72 sm:h-96 object-cover filter brightness-90 transition-transform duration-700 group-hover:scale-105 group-hover:brightness-100"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0B0B0B]/80 via-transparent to-transparent" />
                    
                    {/* Mobile Step Badge */}
                    <div className="md:hidden absolute top-4 left-4 bg-[#0B0B0B]/90 border border-[#C8A96B]/50 px-3 py-1 text-xs font-serif text-[#C8A96B]">
                      STAGE {stepItem.number}
                    </div>
                  </div>
                </div>

                {/* Content Column */}
                <div className={`w-full md:w-1/2 space-y-4 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                  ...
                  <p className="font-sans text-sm text-[#F4F0E8]/75 font-light leading-relaxed">
                    {stepItem.description}
                  </p>

                  {/* Micro checklist details */}
                  <ul className={`pt-2 space-y-1.5 ${isEven ? 'md:flex md:flex-col md:items-end' : ''}`}>
                    {stepItem.details.map((detail, dIdx) => (
                      <li key={dIdx} className="flex items-center space-x-2 text-xs font-sans text-[#FAF8F3]/80 font-light">
